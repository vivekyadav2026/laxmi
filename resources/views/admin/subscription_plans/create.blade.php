...
@section('title', 'Create Subscription Plan - Admin Panel')
...
@section('content')
    <div class="mb-8 relative z-10">
        ...
        <h1 class="text-3xl font-extrabold text-white font-serif mb-1 drop-shadow-md">Create New Plan</h1>
        <p class="text-sm text-gray-400">Configure a new subscription tier with prices, features, and marketing badges.</p>
    </div>
    ...
    <div class="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 p-6 md:p-8 max-w-3xl relative z-10 shadow-2xl">
        <form action="{{ route('admin.subscription_plans.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" required value="{{ old('name') }}" placeholder="e.g. Monthly Plan" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-gold transition-colors">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="slug" required value="{{ old('slug') }}" placeholder="e.g. monthly" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-gold transition-colors font-mono">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="price" id="price" required min="0" value="{{ old('price') }}" placeholder="e.g. 999" class="w-full bg-white/5 border border-white/10 rounded-xl pl-8 pr-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-gold transition-colors">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select name="billing_period" id="billing_period" required class="w-full bg-navy/90 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-gold transition-colors">
                        <option value="month" {{ old('billing_period') == 'month' ? 'selected' : '' }}>Per Month</option>
                        <option value="year" {{ old('billing_period') == 'year' ? 'selected' : '' }}>Per Year</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="description" id="description" value="{{ old('description') }}" placeholder="e.g. Flexibility to Grow" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-gold transition-colors">
                </div>

                <!-- Badge -->
                <div>
                    <label for="badge" class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">Marketing Badge (Optional)</label>
                    <input type="text" name="badge" id="badge" value="{{ old('badge') }}" placeholder="e.g. Most Popular, Best Value" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-gold transition-colors">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="features" id="features" required rows="6" placeholder="e.g.&#10;1 Monthly Mentorship Call&#10;Basic Pitch Deck Review&#10;Loan Application Support" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-gold transition-colors leading-relaxed">{{ old('features') }}</textarea>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            nameInput.addEventListener('input', function () {
                // If slug has not been edited manually, auto-populate it
